...
from .forms import CourseEditForm, ContactForm
...
def contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Thanks! Your message was sent successfully. We\'ll reply soon.')
            return redirect('contact')  # PRG pattern
        else:
            messages.error(request, 'Please correct the errors below and resubmit.')
    else:
        # Pre-fill if user authenticated
        initial = {}
